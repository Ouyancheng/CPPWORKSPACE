...
//	}
	...
	static void quickSort(Iter begin, Iter end, C compare) {
		...
		while (true) {
			...
			while ((compare(*i, *pivot) || i == pivot) && i != j) i++;
			if (i == j) break;
			swap(*i, *j);
		}
		swap(*pivot, *i);
...
private:
	...
	template <typename Iter>
	static inline void swapIter(Iter a, Iter b) {
		typename Sort::PointToType<Iter>::value_type temp = *a;
		(*a) = (*b);
		(*b) =temp;
	}
